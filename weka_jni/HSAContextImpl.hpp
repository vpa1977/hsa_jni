...
public:
	...
	size_t GetComputeUnits() {return 6;}
	size_t GetMaxWorkgroup() {return 512;}
...
private:
	...
	public:
		...
		KernelImpl(hsa_ext_program_handle_t program,hsa_ext_code_descriptor_t* _hsaCodeDescriptor, HSAContextKaveriImpl* _context);
		virtual ~KernelImpl();
	}; // end of KernelImpl
...
public:
	...
	Kernel* createKernel(const char *brig_file, const char *entryName); hsa_status_t dispose();
...
template <typename T>
TemplateDispatch<T>::TemplateDispatch(HSAContext::Kernel* p_kernel) : m_p_kernel(p_kernel)
{
	hsa_region_t region;
	HSAContextKaveriImpl::KernelImpl  *p_impl =(HSAContextKaveriImpl::KernelImpl *) m_p_kernel;
	run_kernel_arg_buffer_size = p_impl->hsaCodeDescriptor->kernarg_segment_byte_size;
...
template <typename T>
void TemplateDispatch<T>::dispatchKernel(T run_args, hsa_signal_t& signal,  const Launch_params_t lparm)
{
    ...
    run_Aql.group_segment_size=p_impl->hsaCodeDescriptor->workgroup_group_segment_byte_size;
    run_Aql.private_segment_size=p_impl->hsaCodeDescriptor->workitem_private_segment_byte_size;
    ...
    uint64_t index = hsa_queue_load_write_index_relaxed(p_impl->context->commandQueue);
    ...
    ((hsa_dispatch_packet_t*)(p_impl->context->commandQueue->base_address))[index&queueMask]=run_Aql;
