...
struct dense_offheap_buffer {
	...
	dense_offheap_buffer(JNIEnv *env, jobject buffer) {
		...
		static jfieldID weights_field = env->GetFieldID(clazz, "m_weights", "J");
		...
		void* weights = (void*)env->GetLongField(buffer, weights_field);

		m_weights.resize(rows);
		memcpy(&m_weights[0], weights, rows * sizeof(value_type));
	...
	std::vector<double>& weight_values_std() {
		return m_weights;
	}
