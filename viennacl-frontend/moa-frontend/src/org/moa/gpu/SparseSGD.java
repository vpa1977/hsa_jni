...
public class SparseSGD extends AbstractClassifier implements NativeClassifier {
    ...
    SparseOffHeapBuffer m_native_batch;
    ...
    public void trainOnInstanceImpl(Instance inst) {
        if (!m_native_init) {
            m_copy_thread = new ThreadPoolExecutor(1, 1, 0, TimeUnit.MILLISECONDS, new ArrayBlockingQueue<Runnable>(QUEUE_SIZE-1));
            m_train_thread = new ThreadPoolExecutor(1, 1, 0, TimeUnit.MILLISECONDS, new ArrayBlockingQueue<Runnable>(QUEUE_SIZE-1));
            m_heaps = new ArrayBlockingQueue<SparseOffHeapBuffer>(QUEUE_SIZE);
            for (int i = 0; i < QUEUE_SIZE - 1; ++i) {
                m_heaps.add(new SparseOffHeapBuffer(m_batch_size, inst.dataset().numAttributes()));
            }
            m_row = 0;
            m_native_batch = new SparseOffHeapBuffer(m_batch_size, inst.dataset().numAttributes());
            initNative(inst.numAttributes(), m_batch_size, m_loss, inst.classAttribute().isNominal(), m_learning_rate, m_lambda);
            m_native_init = true;
        }
