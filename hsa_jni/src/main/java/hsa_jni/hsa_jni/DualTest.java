	...
	//	int test_size  = Integer.parseInt(args[2]);
		...
		for (FirstKnnExperiment experiment : knns )
		{
			...
			try {
				...
				KnnGpuClassifier classifier = new KnnGpuClassifier(context,experiment.getWindow(), experiment.getK());
				
				IBk kMeans = new IBk(experiment.getK());
				kMeans.setNearestNeighbourSearchAlgorithm(new LinearNNSearch());
				...
				DualClassifier dual = new DualClassifier(wekaClassifier, classifier);
