 ...
 * @version $Revision: 7 $
 */
public class ParallelEvaluatePeriodicHeldOutTest extends MainTask {
    ...
    protected Object doMainTask(TaskMonitor monitor, ObjectRepository repository) {
        ParallelClassifier learner = (ParallelClassifier) getPreparedClassOption(this.learnerOption);
	    ...
	    if (this.cacheTestOption.isSet()) {
            ...
            for (instCount = 0; instCount < testSize; instCount++) {
                ...
                Prediction p = new Prediction(testInst, trueClass);
                learner.evaluate(p);
                // try to get complete prediction
                p = learner.poll();
                if (p != null ) 
                {
                	testInst = p.instance();
                	testInst.setClassValue(p.getTrueClass());
                	evaluator.addResult(testInst, p.getVotes());
        	...
        	// finish testing
        	Prediction p;
            while ((p= learner.take() )!= null ) 
            {
            	Instance testInst = p.instance();
            	testInst.setClassValue(p.getTrueClass());
            	evaluator.addResult(testInst, p.getVotes());
            	testInstancesProcessed++;
                if (testInstancesProcessed % INSTANCES_BETWEEN_MONITOR_UPDATES == 0) {
    ...
    public static void main(String[] args)
    {
		...
		test.learnerOption.setCurrentObject(cls);
		...
		ParallelEvaluatePeriodicHeldOutTest test1 = new ParallelEvaluatePeriodicHeldOutTest();
		test1.streamOption.setCurrentObject(generator);
		test1.learnerOption.setCurrentObject(new SampleParallelClassifier());
		test1.testSizeOption.setValue(test_size);
		test1.trainSizeOption.setValue(train_size);
		System.out.println("------Classifier:"+ test.learnerOption.getValueAsCLIString()  );
